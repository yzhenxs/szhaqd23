...
export default {
  ...
  data() {
    return {
      goods: {},
      buyCount: 1, //这里是商品购买数量
      isShowComment: false, //是否显示商品评论
      commentList: [], //评论列表
      total: 0, //总页数
      pageIndex: 1, //页码
      pageSize: 2 ,//页容量
      addToShopCartOffset:null,//加入购物车元素的偏移量
  ...
  methods: {
    ...
    //   console.log(content)
      if (content == '') {
        ...
        return
        //这里要添加个return,这样的话当提交内容为空的时候,不会执行后面的代码
      }
    ...
    addToShopCart(){
        ...
        this.$store.commit('addGoods',goods)//这句话不是很理解
    },
    //动画相关
